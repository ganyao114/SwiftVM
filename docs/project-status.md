...
| SVM_ADDR_EA_TIE | 0/1 | 0 | 寻址修复点 A（`2820ec8`）：A1 GetOperand 终端别名 RA tie 消 mov；A2 identity 专用 composite EA 结构直进 memory IR（bias 逐字节不变）。orb 实测 identity A1+A2 coremark −2.83%/smallpt −0.79% 动态 host 字节 |
