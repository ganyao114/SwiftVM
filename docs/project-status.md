...
**寻址模式发射逐 PC 审计收官（2026-08-04，报告 `/private/tmp/addressing-audit-report.md`，audit-first 无生产改动）**：模式真值（审计时点，当日晚些时候 `3a02f32` 已把 Linux 默认翻为 direct）——两平台默认都是 32-bit bounded bias（`host = pt + (guestEA mod 2^32)`，隔离设计：野指针触不到宿主），`SVM_MEM_DIRECT=1` 是 Linux-only opt-in 恒等映射，macOS 无此开关。差距三层：①非 bias 路径单条融合早已齐全（`[base,index,LSL#n]`/`[base,imm]`/post-index）；②**bias 下 `pt+base+index` 三输入超 ARM64 两寄存器寻址槽，两条是结构下限**（HOSTBASE_FOLD 后实际形态 = wrapping W add + `[pt,W,uxtw]`）；③标量 composite EA 在 `GetOperand` 提前物化导致 emitter 拿不回 base/index 结构。地址路径多余指令动态账：coremark 3.21% / smallpt 7.03% / STREAM 6.59%（观测上界）。修复点分账：**A** simple GetOperand RA tie（coremark **1.148%**/smallpt 0.254%，当前模式干净可修，与 move 桶重叠）；**C** bias 大立即数 guard 误用 LS SIMM9 范围（smallpt 0.097% peephole）；**D** Linux direct（STREAM 池全部）；**G** smallpt 绝对常量物化 3.93%（非寻址题，需 ADRP/literal/reloc 独立设计）；atomic/TSO bare-address 限制双方共有。**orb 实测 `SVM_MEM_DIRECT=1`：STREAM 四稳态循环每条访存精确消一条（Copy 42→32/Scale 28→26/Add 30→27/Triad 32→29 host 指令），oracle×3 全绿（CRC 0x25b5/Validates/SHA）**；direct 还释放 x24(pt)+x10(mem_scratch) 两个保留寄存器（与 pin3 scratch 池问题直接相关）。FEX 取证：其 frontend/IR 一直保留 Base/Index/Scale 结构、无 pt bias（Addressing.cpp/MemoryOps.cpp），恒等是它的天然工作点。**修复点 A 已落地并转正（`2820ec8`/`4400aeb`）：A1 实测 coremark −1.06%（审计池 1.148% 内）、smallpt −0.19%（池 0.254% 内）；A2 direct 复合结构保留追加 coremark −1.76%/smallpt −0.60%（direct 合计 −2.83%/−0.79%），spill 零回涨。修复点 G 已落地（`11d8ce7`，`SVM_ABS_CONST_MAT` 当时默认 OFF（现由 `b692fca` 翻为默认 ON））：绝对常量第三条实为 scratch→result transport（非位宽所需），直发 result 后 smallpt 动态 −1.62%(bias)/−1.35%(direct)、静态 −959 条零增长 PC、spill 零回涨；吃掉旧 G 分子 35.4%，残余 ~2.53% 需 ADRP/literal 基建（两路线均已实测否决：code cache 超 ±4GiB 无 publish-time relocation / FinalizeCode 排池扩出 allocation + serializer 拒 literal，重做需 allocation 与 code/data 元数据重构）。残余：C（0.097% peephole）。**
...
- **W53 Linux 恒等映射（`2b06ff4`，`SVM_MEM_DIRECT`，仅 Linux）**：ET_EXEC PT_LOAD span 按链接地址 MAP_FIXED_NOREPLACE（严格返回地址校验拒绝 pre-4.17 内核），冲突 fatal 不做静默 bias 回退；释放 x24+x10（实测两寄存器 reservation 归零，dispatcher loc x9→x24）；coremark unit 126→110；smallpt 1693 unit 97851→97425 且 image SHA-256 不变；隔离损失已文档化（FEX 同模型），macOS 永不读该开关。捆绑 Linux bring-up 修复：code_cache RW/RX memfd 双映射（修 Linux SEGV_ACCERR）、glibc uc_mcontext、SYS_*/CLONE_* 宏冲突、GNU as bfc→and 等。
- **合并后 Linux 复验（orb Ubuntu）**：master `feb2ec2` 在 orb 重新构建，hello rc=42、func_tests 三模式 × SVM_MEM_DIRECT 两态 × SVM_IR_UNIFORM_RANGE 两态全 PASS（对照 native.txt，rc=101）。W51+W52+W53+W50 交互在 Linux 首次验证。
- kGetenvNames 现 47 项。待翻盘开关队列：SVM_VEC_SHUFPS_NEON、SVM_SMC_DIRTY_HINT、SVM_FLAGS_FCMP_COMPACT、SVM_JIT_CACHE_EXEC_ID、SVM_JIT_SCRATCH_XPOOL、SVM_IR_UNIFORM_RANGE、SVM_MEM_DIRECT（Linux 侧翻盘）。
...
- **W55 pin RAX/RCX/RDX（`82f7e8d`，`SVM_X86_PIN_EXT`）**：W49 计划 B+G.2——x22/x23 callee-saved 零边界成本、x29 随 LR 保存；AH/CH/DH 单条 BFI、32 位写 W-write 自然零扩展；ZeroExtend32To64 单 pinned consumer 融合；窄 pinned read 无中间写时直接以 pinned W 参与 AND/XOR/ZeroExtend（SetHost 交叉检查保 capture）；uniform pass 对新 pin 启用 range-local facts + load-to-load forwarding、DSE 前置保死写删除；Add/Sub scratch 预算 ON +1。orchestrator 自建复验：4400 host unit 净 −3642（1639 减/403 增/2358 不变）、CRC 0x403880 61→62（pin→SSA capture materialization 抵消，留待阶段 2 闭合）、reg mask x22/23/29 reserved、allocatable 14→12；swift_test 四态（OFF/ON/ON+XPOOL/ON+URANGE）122 cases 全绿、func_tests 九格 PASS、sigreturn stale-frame 两态 PASS、指纹 OFF 零 diff；orb Linux 六格（含 PIN+XPOOL、PIN+DIRECT）PASS。codex 受载 A/B coremark +13~14% 仅作方向。
...
- **GPR 全 pin（level 3）× direct 复测（2026-08-04，orb 实测不翻盘，轴维持关闭）**：动机是 direct 把 x24(pt)+x10(mem_scratch) 还回 RA 池（`trampolines.cpp:75-80` 仅 bias 下 Mark；orb `[svm-reg-mask]` 实测 allocatable_gprs：L2i=14、L3i=10、L3b=8）。coremark 150k 三轮中位数：**L2×direct 12,723.7 / L3×direct 9,084.9 / L3×bias 10,222.9 Iterations/Sec**（9/9 CRC `0x25b5` 绿）。level 3 在 direct 下 −28.60% vs level 2，比 bias 下的 −19.66% 更差——+2 寄存器没救回 scratch 枯竭，direct 的融合寻址把 index 寄存器压进池内活跃区间，在 10 寄存器池上反噬。level 2 + XPOOL 维持生产形态。附带发现既有 bug：level 3 + `SVM_EXEC_PROF=1` 在两种内存模式下均 PANIC（"spilling unit reached emission without conditional x18 reservation"），与 direct 无关，level 3 为 opt-in 故仅记录。**2026-08-04 复证该 bug 非 level 3 特有：level 2 默认形态下 smallpt × bias × `SVM_EXEC_PROF=1` 同样命中（新旧两版 master 二进制均复现，direct 模式因多两寄存器不触发 spilling unit 而不崩）；即 EXEC_PROF 抬高 scratch 需求把个别 unit 推进 spilling 路径后缺 x18 条件保留。生产路径（EXEC_PROF 默认 OFF）不受影响，仅记录待修。**
...
| SVM_MEM_DIRECT | 0/OFF/off/1 | Linux 1（mac 无效） | Linux 默认恒等映射（guest=host 原地址寻址）；=0/OFF/off 显式回退 bounded bias（隔离现场开关）。macOS 不读取、永远 bias。撞图 WARNING 后回退 32-bit 窗口续跑 |
| SVM_MEM_MODE_TRACE | 0/1 | 0 | 加载完成后打印实际 direct/use_memory_base/mask/window/bias 诊断 |
| SVM_EXEC_TRACE | 0/1 | 0 | 故障轨迹探针：每 Runtime 32 项 guest RIP/RSP ring，unhandled fault 时转储（`8386d00`）；默认零开销零行为变化 |
| SVM_ADDR_EA_TIE | 0/1 | 1 | 寻址修复点 A（`2820ec8` 落地、`4400aeb` 转正）：A1 GetOperand 终端别名 RA tie 消 mov；A2 direct 专用 composite EA 结构直进 memory IR（bias 逐字节不变）。=0 现场回退（精确复现旧 IR 形态）。orb 实测 direct A1+A2 coremark −2.83%/smallpt −0.79% 动态 host 字节 |
| SVM_ABS_CONST_MAT | 0/1 | 1 | 修复点 G（`11d8ce7`，`b692fca` 翻默认）：绝对地址常量直接向 GetOperand 结果物化，删 scratch→result transport。当前 orb direct smallpt 动态 −1.87%，spill 零回涨；=0 回退旧代码形状 |
...
**默认态变更（2026-08-04，`3a02f32`）**：Linux 默认翻转为 direct 恒等映射
（恢复原地址寻址设计意图；pt/窗口只是开关的本意就是只为特殊地址空间/软页表
开启）。**Linux 默认态下本节的 bounded 隔离属性不再成立**——guest 野指针可达
宿主既有映射；需要隔离的部署必须显式 `SVM_MEM_DIRECT=0`（与
...
落页），配合 `SignalHandler::SetGuestRangeCheck` 的**区间**探针——一次间接调用
...
- **裁决:净动态下界 CoreMark 0.679715% / sqlite 2.291082%,远低于 8% 门,
  ...
  再扣真实新成本后的净账取代。
- 余留唯一活口:PF/AF split 可作为默认 OFF 实验,前置 = direct 下 x24
...
capture 正确性)、signal 非主因、move 桶随 size 换主导(8192B 时 SHA
...
Linux direct 全 pin 首轮池 10,结构上限 12(+x25 RSB / +x27 unpin,
...
- **路一 基建压缩(docs/w66-infra-reclaim-migration.md)**:净账矩阵
  {x27 unpin, x25 unpin, 两者} × {迁最热/次热家},Linux direct 三语料
...
## 2026-08-18 Linux direct 代码质量优化方案定稿

存证 docs/codegen-opt-recipe-2026-08.md,取代
docs/fex-codegen-gap-recipe-2026-08.md 为现行战役。orb Linux direct +
