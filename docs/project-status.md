...
- 下一步：三开关的 orb Linux 墙钟 A/B（安静窗口 7+ 配对）裁定是否翻默认 ON；STREAM 残差 C1（self-edge uniform hoist，依赖 W81）仍在队列。

**direct link v2 重启：设计审计 → P0-A/P0-B 基建 → P1 生产接线 → P2 条件双 exit（2026-08-03，codex 审计/实现 + orchestrator 逐条复验）**：
...
- 后续阶段（审计 §12）：P3 disk cache v4（relocation 表须容纳 unit 内任意数量 nested site 并重放 site kind；revive 先恢复全部 `BL region_tramp` 再逐条注册、最后发布 target，不序列化 Linked 态）。P2 遗留观察项：条件覆盖使 signal retained storage 升至主语料约 1MB logical/AddressSpace、max in-degree 6，长期高 churn 待 Linux 真实负载观察；Switch/CheckHalt 主语料 site=0，扩大覆盖前补专门 corpus。全程默认 OFF，硬回归门不过则停在 opt-in。
