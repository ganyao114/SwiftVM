...
**direct link v2 重启：设计审计 + P0-A 基建落地（2026-08-03，codex 审计/实现 + orchestrator 逐条复验）**：
...
- 后续阶段（审计 §12）：P0-B 跳板+x30 链+epoch/ISB+并发压测 → P1 无条件静态 exit 上生产发码（与 disk cache 互斥）→ P2 条件 terminal 双 exit → P3 disk cache v4。全程默认 OFF，硬回归门不过则停在 opt-in。
