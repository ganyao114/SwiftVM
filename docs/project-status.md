...
- kGetenvNames 现 47 项。待翻盘开关队列：SVM_VEC_SHUFPS_NEON、SVM_SMC_DIRTY_HINT、SVM_FLAGS_FCMP_COMPACT、SVM_JIT_CACHE_EXEC_ID、SVM_JIT_SCRATCH_XPOOL、SVM_IR_UNIFORM_RANGE、SVM_MEM_IDENTITY（Linux 侧翻盘）。进行中：W54 helper ABI 纯值化（RDTSC/XGETBV/WRPKRU/RDRAND 不碰 context）+ W55 pin RAX/RCX/RDX（BFI partial-write + fixed-operand 消费，W49 计划 B+G.2）。
