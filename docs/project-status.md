...
**direct link v2 重启：设计审计 → P0-A/P0-B 基建 → P1 生产接线（2026-08-03，codex 审计/实现 + orchestrator 逐条复验）**：
...
- 后续阶段（审计 §12）：P2 条件 terminal 双 exit（coremark 42.2%/smallpt 22.26% 残差所在，新验收门=能否实际吃掉该残差且保住 P1 signal 事务）→ P3 disk cache v4。全程默认 OFF，硬回归门不过则停在 opt-in。
