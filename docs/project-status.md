...
**direct link v2 重启：设计审计 → P0-A/P0-B 基建 → P1 生产接线 → P2 条件双 exit → P3 disk cache v4（2026-08-03，codex 审计/实现 + orchestrator 逐条复验，全阶段已落地）**：
...
- 后续：P0–P3 全部落地，direct link v2 机制链完整（4B site/跳板/epoch-ISB/signal 摘链/条件双臂/盘 cache 共存），仍默认 OFF。翻默认 ON 的前置 = orb 安静窗口墙钟 A/B（#171 监视中）+ 更宽语料观察 signal retained storage（主语料约 1MB logical/AddressSpace、max in-degree 6）。
