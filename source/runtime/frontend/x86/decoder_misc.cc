...
void X64Decoder::DecodeCpuid(_DInst& insn) {
    ...
    // implemented scalar facilities below. AVX2 / AVX-512 / BMI / ERMS and
    // MMX are deliberately NOT reported so glibc's ifunc dispatch selects the
    // covered SSE2 paths.
    static constexpr u32 kSse2Edx = (1u << 0)   // FPU
                                    ...
                                    | (1u << 25)  // SSE
                                    | (1u << 26); // SSE2
    static constexpr u32 kLeaf1Ecx = (1u << 22)  // MOVBE
                                     | (1u << 30); // RDRAND
    ...
    emit(1, {0x000306C3, 0, kLeaf1Ecx, kSse2Edx});
