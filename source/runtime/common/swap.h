...
template <typename T, typename F> struct swap_struct_t {
...
public:
    ...
    template <typename S> swapped_t& operator=(const S& source) {
    ...
    template <typename S> swapped_t operator/(const S& i) const { return swap() / i; }
    ...
    // v + 5
    swapped_t operator+(const swapped_t& i) const { return swap() + i.swap(); }
    template <typename S> swapped_t operator+(const S& i) const {
    ...
    template <typename S> swapped_t& operator+=(const S& i) {
    ...
    template <typename S> bool operator==(const S& i) const { return swap() == i; }
    ...
    template <typename S> swapped_t operator&(const S& b) const { return swap() & b; }
    swapped_t& operator&=(const swapped_t& b) {
        value = swap(swap() & b.swap());
        return *this;
    }
    template <typename S> swapped_t& operator&=(const S b) {
        value = swap(swap() & b);
        return *this;
    }
    ...
    template <typename S> swapped_t operator<<(const S& b) const { return swap() << b; }
    template <typename S> swapped_t& operator<<=(const S& b) const {
        value = swap(swap() << b);
        return *this;
    }
...
// Arithmetic
template <typename S, typename T, typename F> S operator+(const S& i, const swap_struct_t<T, F> v) {
    return i + v.swap();
}
...
// Arithmetic + assignments
template <typename S, typename T, typename F> S& operator+=(S& i, const swap_struct_t<T, F> v) {
    i += v.swap();
    return i;
}
...
template <typename T> struct swap_64_t {
    static T swap(T x) { return static_cast<T>(swift::runtime::swap64(x)); }
};

template <typename T> struct swap_32_t {
    static T swap(T x) { return static_cast<T>(swift::runtime::swap32(x)); }
};

template <typename T> struct swap_16_t {
    static T swap(T x) { return static_cast<T>(swift::runtime::swap16(x)); }
};

template <typename T> struct swap_float_t {
    static T swap(T x) { return static_cast<T>(swift::runtime::swapf(x)); }
};

template <typename T> struct swap_double_t {
    static T swap(T x) { return static_cast<T>(swift::runtime::swapd(x)); }
};

template <typename T> struct swap_enum_t {
...
template <typename T, typename Tag> struct AddEndian;

// KeepTag specializations

template <typename T> struct AddEndian<T, KeepTag> {
    using type = T;
};

// SwapTag specializations

template <> struct AddEndian<u8, SwapTag> {
    using type = u8;
};

template <> struct AddEndian<u16, SwapTag> {
    using type = swap_struct_t<u16, swap_16_t<u16>>;
};

template <> struct AddEndian<u32, SwapTag> {
    using type = swap_struct_t<u32, swap_32_t<u32>>;
};

template <> struct AddEndian<u64, SwapTag> {
    using type = swap_struct_t<u64, swap_64_t<u64>>;
};

template <> struct AddEndian<s8, SwapTag> {
    using type = s8;
};

template <> struct AddEndian<s16, SwapTag> {
    using type = swap_struct_t<s16, swap_16_t<s16>>;
};

template <> struct AddEndian<s32, SwapTag> {
    using type = swap_struct_t<s32, swap_32_t<s32>>;
};

template <> struct AddEndian<s64, SwapTag> {
    using type = swap_struct_t<s64, swap_64_t<s64>>;
};

template <> struct AddEndian<float, SwapTag> {
    using type = swap_struct_t<float, swap_float_t<float>>;
};

template <> struct AddEndian<double, SwapTag> {
    using type = swap_struct_t<double, swap_double_t<double>>;
};

template <typename T> struct AddEndian<T, SwapTag> {
    static_assert(std::is_enum_v<T>);
...
template <typename T> using enum_le =
...
template <typename T> using enum_be =
