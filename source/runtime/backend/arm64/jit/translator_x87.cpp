...
void JitTranslator::EmitX87Op(ir::Inst* inst) {
    ...
    auto load_top = [&](const Register& destination,
                        const Register& loaded_fsw = Register{}) {
        // TOP is instruction-local. Current inline consumers already hold FSW
        // and therefore need one UBFX; a future/current caller without it can
        // use the same scratch for the architectural LDRH+UBFX pair.
        if (loaded_fsw.IsValid()) {
            __ Ubfx(destination.W(), loaded_fsw.W(), 11, 3);
        } else {
            __ Ldrh(destination.W(), MemOperand(state, kFsw));
            __ Ubfx(destination.W(), destination.W(), 11, 3);
        }
    };
