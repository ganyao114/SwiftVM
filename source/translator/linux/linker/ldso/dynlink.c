...
#include "dynlibc.h"
#include "dynlink.h"
#include "translator/linux/linker/linker_bridge.h"
...
static void __acquire_ptc()
{
    pthread_rwlock_rdlock(&lock);
}

static void __release_ptc()
{
...
static size_t ldso_page_size = 4096;
...
static int error_impl(const char *, ...);
...
struct dso {
	...
	size_t *got;
    struct swift_linux_host_so *host_dso;
	char buf[];
};
...
static void *dl_mmap(size_t n)
{
    ...
    p = (void *)mmap(0, n, prot, flags, -1, 0);
...
static void reloc_all(struct dso *p)
{
	...
	for (; p; p=p->next) {
		...
		if (head != &ldso && p->relro_start != p->relro_end) {
			long ret = mprotect(laddr(p, p->relro_start),
...
static int error_impl(const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	if (!runtime) {
